...
double pwm_freq= 406.5;//417.3;//pwm signal frequency
...
static double mass_main = 9;
static double mass_sub1 = 10.0;
...
static double tau_y_limit = 1.5; // 1.0 -> 1.5 ->3.0 ->1.5 ->1.0 -> 0.5 -> 0.75
static double tau_y_th_limit = 6.0; //2023.08.17 update
...
void setMoI(){
  ...
  hat_CoM_x_sub1 <<
           0,  -CoM.z,    CoM.y+2*l_module,
   CoM.z,           0,              -CoM.x,
  -(CoM.y+2*l_module),   CoM.x,          0;

  hat_CoM_x_sub2 <<
           0,  -CoM.z,                CoM.y,
   CoM.z,           0,  -(CoM.x+2*l_module),
  -CoM.y,    CoM.x-2*l_module,            0;
